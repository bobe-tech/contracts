...
/// @custom:oz-upgrades-from SwapContractV2
contract SwapContractV3 is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    using Math for uint256;
    using SafeERC20 for IERC20;

    // Storage layout MUST match SwapContract V1/V2 exactly (same order, same types).
    ...
    event MainTokenPriceUpdated(uint256 oldPrice, uint256 newPrice);

    event NativeTokenPurchased(
    ...
    function setMainTokenPrice(uint256 newPrice) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(newPrice > 0, "Price must be greater than 0");
        uint256 oldPrice = mainTokenPriceInUsdt;
        mainTokenPriceInUsdt = newPrice;
        emit MainTokenPriceUpdated(oldPrice, newPrice);
    }
    ...
    // --- Public swap functions (V1-compatible wrappers) ---

    function swapNativeToken() external payable nonReentrant {
        _swapNativeToken(msg.sender);
    }

    function swapStableTokens(address token, uint256 amountIn) external nonReentrant {
        _swapStableTokens(msg.sender, token, amountIn);
    }

    function swapAnyTokens(address tokenIn, uint256 amountIn, bytes calldata swapCalldata) external payable nonReentrant {
        _swapAnyTokens(msg.sender, tokenIn, amountIn, swapCalldata);
    }

    // --- New V2 public swap functions with explicit recipient ---

    function swapNativeTokenTo(address recipient) external payable nonReentrant {
        _swapNativeToken(recipient);
    }

    function swapStableTokensTo(address recipient, address token, uint256 amountIn) external nonReentrant {
        _swapStableTokens(recipient, token, amountIn);
    }

    function swapAnyTokensTo(
        address recipient,
        address tokenIn,
        uint256 amountIn,
        bytes calldata swapCalldata
    ) external payable nonReentrant {
        _swapAnyTokens(recipient, tokenIn, amountIn, swapCalldata);
    }

    // --- Internal swap helpers ---

    function _swapNativeToken(address recipient) internal {
        ...
        IERC20(mainTokenAddress).safeTransfer(recipient, mainTokenAmount);

        emit NativeTokenPurchased(msg.sender, recipient, tokenAmount, usdtValue, mainTokenAmount);
    }

    function _swapStableTokens(address recipient, address token, uint256 amountIn) internal {
        require(recipient != address(0), "Recipient cannot be zero address");
        ...
        emit TokensPurchased(msg.sender, recipient, token, actualAmountIn, usdtValue, mainTokenAmount);
    }

    function _swapAnyTokens(
        address recipient,
        address tokenIn,
        uint256 amountIn,
        bytes calldata swapCalldata
        ...
        emit TokensPurchased(msg.sender, recipient, tokenIn, actualAmountIn, usdtReceived, mainTokenAmount);
    }
}
